...
FALCOR_PY_EXPORT(denoisers_ngx_types)
{
    ...
    nb::class_<DLSSOptimalSettings>(ngx_module, "DLSSOptimalSettings", D(ngx, DLSSOptimalSettings))
        .def(nb::init<>())
        .def_rw("render_width", &DLSSOptimalSettings::render_width, D(ngx, DLSSOptimalSettings, render_width))
        .def_rw("render_height", &DLSSOptimalSettings::render_height, D(ngx, DLSSOptimalSettings, render_height))
        .def_rw(
            "max_render_width",
            &DLSSOptimalSettings::max_render_width,
            D(ngx, DLSSOptimalSettings, max_render_width)
        )
        .def_rw(
            "max_render_height",
            &DLSSOptimalSettings::max_render_height,
            D(ngx, DLSSOptimalSettings, max_render_height)
        )
        .def_rw(
            "min_render_width",
            &DLSSOptimalSettings::min_render_width,
            D(ngx, DLSSOptimalSettings, min_render_width)
        )
        .def_rw(
            "min_render_height",
            &DLSSOptimalSettings::min_render_height,
            D(ngx, DLSSOptimalSettings, min_render_height)
        )
        .def_rw("sharpness", &DLSSOptimalSettings::sharpness, D(ngx, DLSSOptimalSettings, sharpness));
    ...
    nb::class_<DLSSSRFeatureDesc>(ngx_module, "DLSSSRFeatureDesc", D(ngx, DLSSSRFeatureDesc))
        .def(nb::init<>())
        .def_rw("quality", &DLSSSRFeatureDesc::quality, D(ngx, DLSSSRFeatureDesc, quality))
        .def_rw("render_width", &DLSSSRFeatureDesc::render_width, D(ngx, DLSSSRFeatureDesc, render_width))
        .def_rw("render_height", &DLSSSRFeatureDesc::render_height, D(ngx, DLSSSRFeatureDesc, render_height))
        .def_rw("target_width", &DLSSSRFeatureDesc::target_width, D(ngx, DLSSSRFeatureDesc, target_width))
        .def_rw("target_height", &DLSSSRFeatureDesc::target_height, D(ngx, DLSSSRFeatureDesc, target_height))
        .def_rw("is_hdr", &DLSSSRFeatureDesc::is_hdr, D(ngx, DLSSSRFeatureDesc, is_hdr))
        .def_rw("depth_inverted", &DLSSSRFeatureDesc::depth_inverted, D(ngx, DLSSSRFeatureDesc, depth_inverted))
        .def_rw(
            "motion_vectors_jittered",
            &DLSSSRFeatureDesc::motion_vectors_jittered,
            D(ngx, DLSSSRFeatureDesc, motion_vectors_jittered)
        )
        .def_rw("auto_exposure", &DLSSSRFeatureDesc::auto_exposure, D(ngx, DLSSSRFeatureDesc, auto_exposure))
        .def_rw(
            "enable_output_subrects",
            &DLSSSRFeatureDesc::enable_output_subrects,
            D(ngx, DLSSSRFeatureDesc, enable_output_subrects)
        )
        .def_rw("sharpness", &DLSSSRFeatureDesc::sharpness, D(ngx, DLSSSRFeatureDesc, sharpness));

    nb::class_<DLSSSREvaluateDesc>(ngx_module, "DLSSSREvaluateDesc", D(ngx, DLSSSREvaluateDesc))
        .def(nb::init<>())
        .def_rw("color", &DLSSSREvaluateDesc::color, D(ngx, DLSSSREvaluateDesc, color))
        .def_rw("depth", &DLSSSREvaluateDesc::depth, D(ngx, DLSSSREvaluateDesc, depth))
        .def_rw("motion_vectors", &DLSSSREvaluateDesc::motion_vectors, D(ngx, DLSSSREvaluateDesc, motion_vectors))
        .def_rw("output", &DLSSSREvaluateDesc::output, D(ngx, DLSSSREvaluateDesc, output))
        .def_rw("exposure", &DLSSSREvaluateDesc::exposure, D(ngx, DLSSSREvaluateDesc, exposure))
        .def_rw(
            "render_subrect_width",
            &DLSSSREvaluateDesc::render_subrect_width,
            D(ngx, DLSSSREvaluateDesc, render_subrect_width)
        )
        .def_rw(
            "render_subrect_height",
            &DLSSSREvaluateDesc::render_subrect_height,
            D(ngx, DLSSSREvaluateDesc, render_subrect_height)
        )
        .def_rw("jitter_offset_x", &DLSSSREvaluateDesc::jitter_offset_x, D(ngx, DLSSSREvaluateDesc, jitter_offset_x))
        .def_rw("jitter_offset_y", &DLSSSREvaluateDesc::jitter_offset_y, D(ngx, DLSSSREvaluateDesc, jitter_offset_y))
        .def_rw("reset", &DLSSSREvaluateDesc::reset, D(ngx, DLSSSREvaluateDesc, reset))
        .def_rw(
            "motion_vector_scale_x",
            &DLSSSREvaluateDesc::motion_vector_scale_x,
            D(ngx, DLSSSREvaluateDesc, motion_vector_scale_x)
        )
        .def_rw(
            "motion_vector_scale_y",
            &DLSSSREvaluateDesc::motion_vector_scale_y,
            D(ngx, DLSSSREvaluateDesc, motion_vector_scale_y)
        )
        .def_rw("pre_exposure", &DLSSSREvaluateDesc::pre_exposure, D(ngx, DLSSSREvaluateDesc, pre_exposure))
        .def_rw("exposure_scale", &DLSSSREvaluateDesc::exposure_scale, D(ngx, DLSSSREvaluateDesc, exposure_scale));

    nb::class_<DLSSGFeatureDesc>(ngx_module, "DLSSGFeatureDesc", D(ngx, DLSSGFeatureDesc))
        .def(nb::init<>())
        .def_rw("width", &DLSSGFeatureDesc::width, D(ngx, DLSSGFeatureDesc, width))
        .def_rw("height", &DLSSGFeatureDesc::height, D(ngx, DLSSGFeatureDesc, height))
        .def_rw("render_width", &DLSSGFeatureDesc::render_width, D(ngx, DLSSGFeatureDesc, render_width))
        .def_rw("render_height", &DLSSGFeatureDesc::render_height, D(ngx, DLSSGFeatureDesc, render_height))
        .def_rw("backbuffer_format", &DLSSGFeatureDesc::backbuffer_format, D(ngx, DLSSGFeatureDesc, backbuffer_format));

    nb::class_<DLSSGCameraDesc>(ngx_module, "DLSSGCameraDesc", D(ngx, DLSSGCameraDesc))
        .def(nb::init<>())
        .def_rw(
            "camera_view_to_clip",
            &DLSSGCameraDesc::camera_view_to_clip,
            D(ngx, DLSSGCameraDesc, camera_view_to_clip)
        )
        .def_rw(
            "clip_to_camera_view",
            &DLSSGCameraDesc::clip_to_camera_view,
            D(ngx, DLSSGCameraDesc, clip_to_camera_view)
        )
        .def_rw("clip_to_lens_clip", &DLSSGCameraDesc::clip_to_lens_clip, D(ngx, DLSSGCameraDesc, clip_to_lens_clip))
        .def_rw("clip_to_prev_clip", &DLSSGCameraDesc::clip_to_prev_clip, D(ngx, DLSSGCameraDesc, clip_to_prev_clip))
        .def_rw("prev_clip_to_clip", &DLSSGCameraDesc::prev_clip_to_clip, D(ngx, DLSSGCameraDesc, prev_clip_to_clip))
        .def_rw("jitter_offset", &DLSSGCameraDesc::jitter_offset, D(ngx, DLSSGCameraDesc, jitter_offset))
        .def_rw(
            "motion_vector_scale",
            &DLSSGCameraDesc::motion_vector_scale,
            D(ngx, DLSSGCameraDesc, motion_vector_scale)
        )
        .def_rw(
            "camera_pinhole_offset",
            &DLSSGCameraDesc::camera_pinhole_offset,
            D(ngx, DLSSGCameraDesc, camera_pinhole_offset)
        )
        .def_rw("camera_pos", &DLSSGCameraDesc::camera_pos, D(ngx, DLSSGCameraDesc, camera_pos))
        .def_rw("camera_up", &DLSSGCameraDesc::camera_up, D(ngx, DLSSGCameraDesc, camera_up))
        .def_rw("camera_right", &DLSSGCameraDesc::camera_right, D(ngx, DLSSGCameraDesc, camera_right))
        .def_rw("camera_fwd", &DLSSGCameraDesc::camera_fwd, D(ngx, DLSSGCameraDesc, camera_fwd))
        .def_rw("camera_near", &DLSSGCameraDesc::camera_near, D(ngx, DLSSGCameraDesc, camera_near))
        .def_rw("camera_far", &DLSSGCameraDesc::camera_far, D(ngx, DLSSGCameraDesc, camera_far))
        .def_rw("camera_fov", &DLSSGCameraDesc::camera_fov, D(ngx, DLSSGCameraDesc, camera_fov))
        .def_rw(
            "camera_aspect_ratio",
            &DLSSGCameraDesc::camera_aspect_ratio,
            D(ngx, DLSSGCameraDesc, camera_aspect_ratio)
        )
        .def_rw("color_buffers_hdr", &DLSSGCameraDesc::color_buffers_hdr, D(ngx, DLSSGCameraDesc, color_buffers_hdr))
        .def_rw("depth_inverted", &DLSSGCameraDesc::depth_inverted, D(ngx, DLSSGCameraDesc, depth_inverted))
        .def_rw(
            "camera_motion_included",
            &DLSSGCameraDesc::camera_motion_included,
            D(ngx, DLSSGCameraDesc, camera_motion_included)
        )
        .def_rw("reset", &DLSSGCameraDesc::reset, D(ngx, DLSSGCameraDesc, reset))
        .def_rw(
            "automode_override_reset",
            &DLSSGCameraDesc::automode_override_reset,
            D(ngx, DLSSGCameraDesc, automode_override_reset)
        )
        .def_rw(
            "not_rendering_game_frames",
            &DLSSGCameraDesc::not_rendering_game_frames,
            D(ngx, DLSSGCameraDesc, not_rendering_game_frames)
        )
        .def_rw("ortho_projection", &DLSSGCameraDesc::ortho_projection, D(ngx, DLSSGCameraDesc, ortho_projection))
        .def_rw(
            "motion_vectors_invalid_value",
            &DLSSGCameraDesc::motion_vectors_invalid_value,
            D(ngx, DLSSGCameraDesc, motion_vectors_invalid_value)
        )
        .def_rw(
            "motion_vectors_dilated",
            &DLSSGCameraDesc::motion_vectors_dilated,
            D(ngx, DLSSGCameraDesc, motion_vectors_dilated)
        )
        .def_rw(
            "menu_detection_enabled",
            &DLSSGCameraDesc::menu_detection_enabled,
            D(ngx, DLSSGCameraDesc, menu_detection_enabled)
        )
        .def_rw("multi_frame_count", &DLSSGCameraDesc::multi_frame_count, D(ngx, DLSSGCameraDesc, multi_frame_count))
        .def_rw("multi_frame_index", &DLSSGCameraDesc::multi_frame_index, D(ngx, DLSSGCameraDesc, multi_frame_index));

    nb::class_<DLSSGEvaluateDesc>(ngx_module, "DLSSGEvaluateDesc", D(ngx, DLSSGEvaluateDesc))
        .def(nb::init<>())
        .def_rw("backbuffer", &DLSSGEvaluateDesc::backbuffer, D(ngx, DLSSGEvaluateDesc, backbuffer))
        .def_rw("depth", &DLSSGEvaluateDesc::depth, D(ngx, DLSSGEvaluateDesc, depth))
        .def_rw("motion_vectors", &DLSSGEvaluateDesc::motion_vectors, D(ngx, DLSSGEvaluateDesc, motion_vectors))
        .def_rw(
            "output_interpolated_frame",
            &DLSSGEvaluateDesc::output_interpolated_frame,
            D(ngx, DLSSGEvaluateDesc, output_interpolated_frame)
        )
        .def_rw("camera", &DLSSGEvaluateDesc::camera, D(ngx, DLSSGEvaluateDesc, camera));
}

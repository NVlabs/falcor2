...
This directory owns MaterialX graph preparation passes used before MTLX code
...
checks. Runtime-editable graph inputs enter static analysis as unknown values,
so pruning remains enabled but cannot rely on their authored defaults.
...
MTLX pruning follows OSL-style closure algebra where the meaning is generic
closure composition. It uses local MTLX policy where MaterialX layering or
...
MTLX VDF pruning support is intentionally narrow. VDF constructor nodes such
...
- Direct VDF closure algebra such as `add_vdf`, `mix_vdf`, or `multiply_vdf` is
  not part of MTLX pruning today.

...
ScalarStaticValue collapse_uniform_value(const ScalarStaticValue& value)
{
    if (value.kind != ScalarStaticValueKind::KnownConstant || value.value.size == 0)
        return value;

    const float first = value.value.values[0];
    for (uint8_t i = 1; i < value.value.size; ++i) {
        if (value.value.values[i] != first)
            return ScalarStaticValue::unknown();
    }

    ScalarStaticConstant constant;
    constant.values[0] = first;
    constant.size = 1;
    return ScalarStaticValue::constant(constant);
}

std::optional<ScalarStaticConstant> expand_static_value(const ScalarStaticValue& value, uint8_t size)
{
    ScalarStaticConstant result;
    result.size = size;

    if (value.kind == ScalarStaticValueKind::KnownZero) {
        result.values.fill(0.0f);
    } else if (value.kind == ScalarStaticValueKind::KnownOne) {
        result.values.fill(1.0f);
    } else if (value.kind == ScalarStaticValueKind::KnownConstant && value.value.size == size) {
        result = value.value;
    } else {
        return {};
    }
...
ScalarStaticValue make_static_value(const ScalarStaticConstant& value)
{
    bool all_zero = true;
    bool all_one = true;
    for (uint8_t i = 0; i < value.size; ++i) {
        all_zero = all_zero && value.values[i] == 0.0f;
        all_one = all_one && value.values[i] == 1.0f;
    }
    if (all_zero)
        return ScalarStaticValue::zero();
    if (all_one)
        return ScalarStaticValue::one();
    return ScalarStaticValue::constant(value);
}

bool is_finite(const ScalarStaticConstant& value)
{
    for (uint8_t i = 0; i < value.size; ++i) {
...
ScalarStaticValue evaluate_luminance(const mx::ShaderNode& node, const OutputStaticValueMap& current)
{
    const mx::ShaderInput* input = node.getInput("in");
    const mx::ShaderInput* coefficients = node.getInput("lumacoeffs");
    ...
    const std::optional<ScalarStaticConstant> input_value = expand_static_value(scalar_of(*input, current), input_size);
    const std::optional<ScalarStaticConstant> coefficient_value
        = expand_static_value(scalar_of(*coefficients, current), 3);
    if (!input_value || !coefficient_value || !is_finite(*input_value) || !is_finite(*coefficient_value))
        return ScalarStaticValue::unknown();

    const float luminance = input_value->values[0] * coefficient_value->values[0]
        + input_value->values[1] * coefficient_value->values[1] + input_value->values[2] * coefficient_value->values[2];
    if (!std::isfinite(luminance))
        return ScalarStaticValue::unknown();

    ScalarStaticConstant result;
    result.values = {luminance, luminance, luminance, input_size == 4 ? input_value->values[3] : 0.0f};
    result.size = input_size;
    return make_static_value(result);
}
...
void seed_top_graph_env(
    const mx::ShaderGraph& top,
    OutputStaticValueMap& out,
    const RuntimeInputPredicate& is_runtime_input
)
{
    for (const mx::ShaderOutput* socket : top.getInputSockets()) {
        out[socket] = is_runtime_input(*socket) ? OutputStaticValue{} : socket_default_static_value(socket);
    }
}
    ...
    if (!has_closure_rule) {
        if (is_constant_node(impl)) {
            ...
            if (is_zero_static_value(input1_value))
                ...
                scalar_out = ScalarStaticValue::unknown();
            has_scalar_rule = true;
        } else if (is_luminance_node(impl)) {
            scalar_out = evaluate_luminance(node, current);
            has_scalar_rule = true;
        } else if (is_extract_node(impl)) {
            scalar_out = collapse_uniform_value(scalar_of(node.getInput("in"), current));
            has_scalar_rule = true;
        }
    }
...
)
{
    ...
    for (const mx::ShaderGraph* graph : index.reachable_graphs()) {
        if (graph == &top) {
            seed_top_graph_env(top, current, is_runtime_input);
        } else {
...
{
    return analyze_closure_pruning(top, index, prev_output_static_values, is_runtime_input).static_values;
}

StaticValueAnalysisResult
analyze_static_values_to_fixpoint(const mx::ShaderGraph& top, const RuntimeInputPredicate& is_runtime_input)
{
    ...
    while (true) {
        ...
        result = analyze_static_values(top, index, prev_values, is_runtime_input);
        ++iterations;

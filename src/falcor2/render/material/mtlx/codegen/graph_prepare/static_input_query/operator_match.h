...
// MTLX pruning handles VDF only through layer_vdf boundary policy.
...
inline bool is_luminance_node(std::string_view impl)
{
    return implementation_name_starts_with(impl, "IM_luminance_color");
}

inline bool is_extract_node(std::string_view impl)
{
    return implementation_name_starts_with(impl, "IM_extract_");
}

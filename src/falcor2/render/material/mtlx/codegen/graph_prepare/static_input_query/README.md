# MTLX Static Input Query

This directory owns reusable static input analysis for MTLX MaterialX graphs.
...
- `static_value_analysis.h`: per-output static-value analysis result and the
  static-only analysis entry point, including the predicate that identifies
  runtime-editable top-level inputs.
...
Current non-pruning consumers:

- MTLX Fresnel policy selection in `policy.cpp`, which uses
  `StaticInputQuery` to select the cheapest generalized Schlick Fresnel
  implementation proven valid by exact static values.
- Root opacity classification in `codegen_support/opacity_analysis.cpp`, which
  resolves opacity through expanded implementation graphs and reduces
  surfaceshader composition to an exact scalar when possible.

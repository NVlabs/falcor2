...
inline bool all_components_equal(const int& v, float target)
{
    return static_cast<float>(v) == target;
}

inline bool all_components_equal(const bool& v, float target)
{
    return static_cast<float>(v) == target;
}
...
inline void pack_scalar_static_constant(const int& v, ScalarStaticConstant& out)
{
    out.values = {static_cast<float>(v), 0, 0, 0};
    out.size = 1;
}

inline void pack_scalar_static_constant(const bool& v, ScalarStaticConstant& out)
{
    out.values = {static_cast<float>(v), 0, 0, 0};
    out.size = 1;
}
...
inline ScalarStaticValue lift_static_value(const MaterialX::ValuePtr& value)
{
    if (!value)
        return ScalarStaticValue::unknown();
    if (value->isA<int>())
        return input_static_values_detail::lift_typed_static_value<int>(*value);
    if (value->isA<bool>())
        return input_static_values_detail::lift_typed_static_value<bool>(*value);
    if (value->isA<float>())
...
inline bool has_supported_runtime_value(const MaterialX::ShaderOutput& input)
{
    const MaterialX::ValuePtr& value = input.getValue();
    return value
        && (value->isA<int>() || value->isA<bool>() || value->isA<float>() || value->isA<MaterialX::Color3>()
            || value->isA<MaterialX::Color4>() || value->isA<MaterialX::Vector2>() || value->isA<MaterialX::Vector3>()
            || value->isA<MaterialX::Vector4>());
}

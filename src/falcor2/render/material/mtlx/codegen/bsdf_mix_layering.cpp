...
{
    FALCOR_CHECK(!ref.is_none(), "Mtlx bsdf_mix layering cannot encode a null closure reference.");
    return ref.is_bsdf();
}
...
int ref_index(ClosureRef ref)
{
    ...
    FALCOR_CHECK(ref.is_combiner(), "Mtlx bsdf_mix layering cannot encode a null closure reference.");
...
)
{
    if (is_bsdf_ref(ref)) {
        ...
        FALCOR_CHECK(
            ...
            "Mtlx bsdf_mix sample probability references invalid BSDF {}.",
            bsdf_index
        );
    ...
    FALCOR_CHECK(
        ...
        "Mtlx bsdf_mix sample probability references invalid combiner {}.",
        combiner_index
    );
    FALCOR_CHECK(
        !visiting_combiners[size_t(combiner_index)],
        "Mtlx bsdf_mix sample probability graph contains a cycle."
    );
    visiting_combiners[size_t(combiner_index)] = true;

    const MxFlatCombinerDesc& combiner = desc.combiners[size_t(combiner_index)];
    FALCOR_CHECK(combiner.index == combiner_index, "Mtlx bsdf_mix combiner index mismatch.");
...
)
{
    if (is_bsdf_ref(ref)) {
        ...
        FALCOR_CHECK(
            bsdf_index >= 0 && size_t(bsdf_index) < bsdf_terms.size(),
            "Mtlx bsdf_mix closure weight references invalid BSDF {}.",
            bsdf_index
        );
    ...
    FALCOR_CHECK(
        combiner_index >= 0 && size_t(combiner_index) < desc.combiners.size(),
        "Mtlx bsdf_mix closure weight references invalid combiner {}.",
        combiner_index
    );
    FALCOR_CHECK(!visiting_combiners[size_t(combiner_index)], "Mtlx bsdf_mix closure weight graph contains a cycle.");
    visiting_combiners[size_t(combiner_index)] = true;
...
{
    FALCOR_CHECK(!terms.empty(), "Cannot sum empty Mtlx bsdf_mix term list.");
    std::string result;
...
void emit_eval_with_pdf(SlangEmitter& e, const MxFlatRootDesc& desc)
{
    e.begin(
        "public BSDFEval eval_with_pdf<S : ISampleGenerator>(const float3 wi, const float3 wo, inout S sg, const "
        "BSDFContext bc)"
    );
    e.line("BSDFEval result = {}");
    for (size_t i = 0; i < desc.bsdfs.size(); ++i) {
        const int bsdf_index = int(i);
        const std::string lobe = "lobe_" + std::to_string(i);
        e.line(
            "BSDFEval " + lobe + " = flat::eval_bsdf_with_pdf(" + desc.bsdfs[i].field_name + ", "
            + bsdf_weight_field(bsdf_index) + ", " + bsdf_frame_field(bsdf_index) + ", wi, wo, sg, bc)"
        );
        e.line("result.value += " + lobe + ".value");
        e.line("result.pdf += bsdf_sample_probabilities[" + std::to_string(i) + "] * " + lobe + ".pdf");
    }
...
void emit_sample(SlangEmitter& e)
{
    ...
    e.line("// MTLX bsdf_mix sample selector: direct CDF over final BSDF probabilities.", false);
    e.line("sample = {}");
    e.line();
...
{
    FALCOR_CHECK(!desc.type_name.empty(), "Mtlx bsdf_mix root type name cannot be empty.");
    FALCOR_CHECK(!desc.bsdfs.empty(), "Mtlx bsdf_mix root requires at least one BSDF.");

    SlangEmitter e;
    const size_t bsdf_count = std::max<size_t>(desc.bsdfs.size(), 1);
    const size_t combiner_count = std::max<size_t>(desc.combiners.size(), 1);

    e.line("// Mtlx bsdf_mix layering root generated from the MaterialX closure tree.");
    ...
    e.line("return mtlx::from_scattering_albedo(root_reflection, root_transmission)");
    e.end();

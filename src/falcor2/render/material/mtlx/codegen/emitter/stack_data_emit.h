...
// Callback that returns a target-independent upper bound for the uniform block size. The bound
// selects between fixed-size inline storage and an out-of-line buffer before target reflection is
// available, so it must never be smaller than the reflected size on any supported target.
using CalculateDataSizeUpperBoundFn = std::function<std::size_t(const MaterialX::VariableBlock&)>;

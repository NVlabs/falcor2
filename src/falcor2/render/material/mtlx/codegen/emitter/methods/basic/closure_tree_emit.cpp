...
)
{
    ...
    const bool evaluate_opacity = params.opacity.kind == CodeGenResult::OpacityResult::Kind::dynamic;
    const bool opacity_is_opaque = params.opacity.kind == CodeGenResult::OpacityResult::Kind::opaque;
    const std::string static_opacity = params.opacity.kind == CodeGenResult::OpacityResult::Kind::constant
        ? std::to_string(params.opacity.constant)
        : "1.0";
    ...
    const std::string collect_properties_method
        = strategy->build_collect_properties_text(layering, "    ", is_transmissive);
    ...
    source += codegen_support::render_snippet(
        ...
        codegen_support::TokenMap{
            ...
            {"$MxHasEmission", graph_can_emit ? "1" : "0"},
            {"$MxEvaluateOpacity", evaluate_opacity ? "1" : "0"},
            {"$MxOpacityIsOpaque", opacity_is_opaque ? "1" : "0"},
            {"$MxStaticOpacity", static_opacity},
            {"$MxGraphEmission", graph_emission},
            {"$MxGraphOpacity", graph_opacity},

...
height_to_normal_sample_strings(const mx::ShaderNode& node, mx::GenContext& context, mx::ShaderStage& stage)
{
    ...
    FALCOR_CHECK(in_input, "MTLX heighttonormal node missing in input.");

    const mx::ShaderOutput* in_connection = in_input->getConnection();
    const mx::ShaderOutput* output = node.getOutput();
    FALCOR_CHECK(output, "MTLX heighttonormal node missing output.");
    ...
    if (samples.empty()) {
        FALCOR_CHECK(in_connection || in_input->getValue(), "MTLX heighttonormal node missing in connection or value.");
...
) const
{
    DEFINE_SHADER_STAGE(stage, mx::Stage::PIXEL)
    {
        ...
        const mx::ShaderOutput* output = node.getOutput();
        FALCOR_CHECK(scale_input && output, "MTLX heighttonormal node missing scale input or output.");

        const std::vector<std::string> samples = height_to_normal_sample_strings(node, context, stage);
        FALCOR_CHECK(samples.size() == 9, "MTLX heighttonormal expected 9 samples.");

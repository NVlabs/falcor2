...
resolve_geomprop_id(const CodeGenDesc& desc, std::string_view geomprop, std::string_view output_type)
{
    ...
    return ::falcor::mtlx::test_geomprop_id(geomprop);
}
...
) const
{
    ...
    FALCOR_CHECK(user_data && user_data->inputs.desc, "MTLX geompropvalue node missing user data.");
    ...
    const mx::ShaderOutput* output = node.getOutput();
    FALCOR_CHECK(output, "MTLX geompropvalue node missing output.");
...
{
    auto user_data = context.getUserData<CodegenUserData>("mtlx");
    FALCOR_CHECK(user_data && user_data->inputs.desc, "MTLX texcoord node missing user data.");

    const std::string index = required_shader_input_value_string(node, "index", "0");
    const std::string output_type = shader_output_type_name(node);
    const mx::ShaderOutput* output = node.getOutput();
    FALCOR_CHECK(output, "MTLX texcoord node missing output.");
...
{
    auto user_data = context.getUserData<CodegenUserData>("mtlx");
    FALCOR_CHECK(user_data && user_data->inputs.desc, "MTLX geomcolor node missing user data.");

    const std::string index = required_shader_input_value_string(node, "index", "0");
    const std::string output_type = shader_output_type_name(node);
    const mx::ShaderOutput* output = node.getOutput();
    FALCOR_CHECK(output, "MTLX geomcolor node missing output.");

...
private:
    ...
    void add_node(mx::ShaderNode* node)
    {
        ...
        FALCOR_CHECK(
            outputs.size() == 1,
            "MTLX cannot handle multiple BSDF/surface outputs from node {}.",
            node->getName()
        );
    ...
    static void append_dependent_closure_nodes(std::vector<mx::ShaderNode*>& result, mx::ShaderNode* shader_node)
    {
        if (shader_node->hasClassification(mx::ShaderNode::Classification::MATERIAL)) {
            // MTLX currently emits the concrete MaterialX material through its
    ...
    )
    {
        ...
        if (resolved_inputs.empty())
            ...
            FALCOR_THROW(
                "MTLX node {} has multiple connected BSDF-like inputs but is not a supported composition node.",
                node->getName()
            );
        }
    }
    ...
    )
    {
        ...
        FALCOR_THROW("MTLX node {} is not connected to a known BSDF output.", shader_node->getName());
    }
    ...
    )
    {
        ...
        for (std::size_t i = 0; i < input_names.size(); ++i) {
            ...
            FALCOR_CHECK(
                ...
                "MTLX input {}.{} is not connected to a known BSDF output.",
                shader_node->getName(),
                input_name
            );
            desc.children[i] = ref;
    ...
    combiner_input_names(mx::ShaderNode* shader_node, const std::string& implementation)
    {
        ...
        FALCOR_THROW("MTLX unsupported BSDF combiner implementation '{}'.", implementation);
    }
...
std::unordered_map<std::string, BsdfNodeInfo> make_bsdf_node_map(const ProfileDesc& profile, const CodeGenDesc& desc)
{
    return {
        ...
        {"IM_chiang_hair_bsdf_genslangpt", {profile.bsdf_type("ChiangHair"), "BSDFFlags::glossy_curve", false, true}},
    ...
    for (LayeringDesc::BsdfDesc& bsdf : desc.bsdfs) {
        ...
        if (microfacet_kind(bsdf.node_impl) != MxMicrofacetKind::none) {
            MxMicrofacetSelection selection
                = select_microfacet(codegen_desc, bsdf.node_impl, bsdf.shader_node, constants, static_input_query);
            bsdf.has_microfacet_selection = true;
        ...
        if (is_scatter_mode_bsdf(bsdf.node_impl)) {
            MxMicrofacetSelection selection
                = select_microfacet(codegen_desc, bsdf.node_impl, bsdf.shader_node, constants, static_input_query);
            if (bsdf.has_microfacet_selection) {
...
void validate_weighted_layer_top_refs_are_not_shared(const LayeringDesc& desc)
{
    ...
    for (ClosureRef combiner_ref : visited_combiners) {
        ...
        FALCOR_CHECK(
            ...
            "Mtlx weighted_layer node {} cannot fold top_weight into reused top closure ref {}. "
...
{
    return bsdf.node_impl == k_surface_unlit_transparent_impl;
}

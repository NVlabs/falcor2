...
#include "codegen_support/emission_analysis.h"
...
#include "graph_prepare/static_input_query/input_static_values.h"
...
constexpr const char* k_falcor_working_color_space = "lin_rec709";
...
size_t material_data_alignment_rank(const mx::TypeDesc& type)
{
    // Conservative maximum alignment for generated StructuredBuffer elements on the supported
    // D3D12, Vulkan, and CUDA targets. Sorting by this rank avoids target-specific padding.
    if (type.getSemantic() == mx::TypeDesc::SEMANTIC_MATRIX)
        return 16;
    if (type.getSemantic() == mx::TypeDesc::SEMANTIC_VECTOR || type.getSemantic() == mx::TypeDesc::SEMANTIC_COLOR) {
        if (type.getSize() >= 3)
            return 16;
        if (type.getSize() == 2)
            return 8;
...
class GenslangPtCompoundNode : public mx::CompoundNode {
public:
    explicit GenslangPtCompoundNode(std::string class_function_name)
        : m_class_function_name(std::move(class_function_name))
    {
    }

    void initialize(const mx::InterfaceElement& element, mx::GenContext& context) override
    {
        mx::CompoundNode::initialize(element, context);
        if (!m_class_function_name.empty()) {
            _functionName = m_class_function_name;
            _hash = std::hash<std::string>{}(_functionName);
        }
    }

private:
    std::string m_class_function_name;
};

// MaterialX uses authored node and parameter names in the emitted identifiers. Class compilation
// requires graphs with the same topology to produce the same Slang source even when those names
// differ. Replace shader-visible names with names derived from the graph structure, while recording
// the previous property identity of every input so runtime metadata can remain material-specific.
// The visited set is shared by the recursion because MaterialX nodes may reuse the same
// implementation graph.
void canonicalize_class_graph_bindings(
    mx::ShaderGraph& graph,
    CodegenUserData& user_data,
    std::unordered_set<const mx::ShaderGraph*>& visited_graphs
)
{
    if (!visited_graphs.insert(&graph).second)
        return;

    std::unordered_map<const mx::ShaderNode*, size_t> node_ids;
    std::vector<mx::ShaderNode*> ordered_nodes;

    // Number nodes by a depth-first traversal of their connections rather than by authored name.
    // Each node is numbered when first encountered, giving equivalent topologies the same IDs.
    std::function<void(mx::ShaderNode*)> visit = [&](mx::ShaderNode* node)
    {
        if (!node || node == &graph || node_ids.contains(node))
            return;

        node_ids.emplace(node, ordered_nodes.size());
        ordered_nodes.push_back(node);
        for (mx::ShaderInput* input : node->getInputs()) {
            if (mx::ShaderOutput* connection = input->getConnection())
                visit(connection->getNode());
        }
    };

    // MaterialX retains output sockets in interface order. This positional order is part of the
    // graph structure, unlike the authored socket names, so use it to seed the traversal.
    const auto& output_sockets = graph.getOutputSockets();
    for (mx::ShaderGraphOutputSocket* socket : output_sockets) {
        if (mx::ShaderOutput* connection = socket->getConnection())
            visit(connection->getNode());
    }
    for (mx::ShaderNode* node : graph.getNodes())
        visit(node);

    // MaterialX retains input sockets in interface order. Treat this positional order as part of the
    // graph structure, matching the output interface below.
    const auto& input_sockets = graph.getInputSockets();
    for (size_t i = 0; i < input_sockets.size(); ++i) {
        mx::ShaderGraphInputSocket* socket = input_sockets[i];
        user_data.analysis.pre_canonical_input_names.emplace(socket, socket->getName());
        socket->setVariable(fmt::format("mx_p{}", i));
    }

    // Canonicalize node ports by node ID and port position. Record every input's existing variable
    // name before replacing it, independently of whether the input is currently exposed as a
    // runtime material property.
    for (size_t node_index = 0; node_index < ordered_nodes.size(); ++node_index) {
        mx::ShaderNode* node = ordered_nodes[node_index];
        for (size_t input_index = 0; input_index < node->numInputs(); ++input_index) {
            mx::ShaderInput* input = node->getInput(input_index);
            user_data.analysis.pre_canonical_input_names.emplace(input, input->getVariable());
            input->setVariable(fmt::format("mx_n{}_i{}", node_index, input_index));
        }
        for (size_t output_index = 0; output_index < node->numOutputs(); ++output_index)
            node->getOutput(output_index)->setVariable(fmt::format("mx_n{}_o{}", node_index, output_index));

        // Implementation graphs have their own scopes and must be canonicalized recursively.
        if (mx::ShaderGraph* implementation_graph = node->getImplementation().getGraph())
            canonicalize_class_graph_bindings(*implementation_graph, user_data, visited_graphs);
    }

    // Output identifiers follow the same structural interface order.
    for (size_t i = 0; i < output_sockets.size(); ++i)
        output_sockets[i]->setVariable(fmt::format("mx_o{}", i));
}

void canonicalize_class_graph_bindings(mx::ShaderGraph& graph, CodegenUserData& user_data)
{
    std::unordered_set<const mx::ShaderGraph*> visited_graphs;
    canonicalize_class_graph_bindings(graph, user_data, visited_graphs);
}
...
void register_source_paths(mx::GenContext& context)
{
#if defined(MTLX_STANDARD_LIBRARY_PATH)
    register_source_path_with_parent(context, MTLX_STANDARD_LIBRARY_PATH);
#endif
#if defined(MTLX_LIBRARY_PATH)
    std::filesystem::path mx_library_path = MTLX_LIBRARY_PATH;
...
public:
    ...
    explicit GenslangPtShaderGenerator(
        mx::TypeSystemPtr type_system,
    ...
    mx::ShaderNodeImplPtr createShaderNodeImplForNodeGraph(const mx::NodeGraph&) const override
    {
        std::string class_function_name;
        if (m_class_compilation)
            class_function_name = fmt::format("mx_class_graph_{}", m_class_compound_count++);
        return std::make_shared<GenslangPtCompoundNode>(std::move(class_function_name));
    }

    static mx::ShaderGeneratorPtr
    create(mx::TypeSystemPtr type_system = nullptr, bool use_slang_derivatives = false, bool class_compilation = false)
    {
    ...
    mx::ShaderPtr generate(const std::string& name, mx::ElementPtr element, mx::GenContext& context) const override
    {
        ...
        FALCOR_CHECK(
            ...
            "MTLX generator missing user data."
        );
        ...
        emitLine("namespace mtlx", stage, false);
        ...
        emitter::emit_material_data(
            *this,
            ...
            {
                emit_shader_inputs(inputs, s, public_members, as_property, true);
            },
            [this](const mx::VariableBlock& inputs)
            {
                return calculate_data_size_upper_bound(inputs);
            },
            material_data_name,
            layering,
            context,
        ...
        // local MTLX snippet whenever generated source references mx_space_* symbols.
        ...
        emit_material_structs(
            ...
            user_data->inputs.result->opacity,
        ...
        // Select the Zeltner coefficient factory before source hashing so the
        // specification default and LUT comparison mode have distinct module identities.
        replace_zeltner_sheen_factory_calls(code, *user_data->inputs.desc);
        ...
        result.material_name = "mtlx::" + material_name;
        result.material_instance_name = "mtlx::" + material_instance_name;
        result.material_data_name = "mtlx::" + material_data_name;
        ...
        result.codegen_metadata.set("bsdf_profile", std::string("mtlx"));
...
private:
    ...
    mutable size_t m_class_compound_count = 0;
    ...
    void emit_geomprop_id_constants(const CodeGenResult& result, mx::ShaderStage& stage) const
    {
        ...
        for (const CodeGenResult::GeometryStreamRequest& request : result.required_geomprop_streams) {
            ...
            FALCOR_CHECK(
                ...
                "MTLX geomprop streams produce conflicting shader constant `{}`.",
                const_name
            );
        }
    ...
    mx::ShaderPtr create_shader(const std::string& name, mx::ElementPtr element, mx::GenContext& context) const
    {
        ...
        if (user_data->inputs.desc->class_compilation)
            canonicalize_class_graph_bindings(*graph, *user_data);
        ...
        const CodeGenDesc& desc = *user_data->inputs.desc;
        const OptimizeGraphFlags optimize_graph_flags = effective_optimize_graph_flags(desc);
        ...
        const bool output_can_emit = graph_output_can_emit(outputs);
        const bool needs_static_opacity = output_can_emit;
        const bool needs_static_input_query = needs_static_microfacet_constants || needs_static_opacity;
        ...
        if (needs_static_input_query) {
            const RuntimeInputPredicate is_runtime_input = [&desc](const mx::ShaderOutput& input)
            {
                return has_supported_runtime_value(input) && desc.is_editable_param(input.getName());
            };
            static_values = analyze_static_values_to_fixpoint(*graph, is_runtime_input);
            static_input_query.emplace(static_values.output_static_values, microfacet_constants);
        }

        if (output_can_emit) {
            const std::optional<float> static_opacity = needs_static_opacity && static_input_query
                ? materialx_graph_static_opacity(*graph, *static_input_query)
                : std::optional<float>{};
            if (!static_opacity) {
                result.opacity.kind = CodeGenResult::OpacityResult::Kind::dynamic;
            } else {
                result.opacity.constant = *static_opacity;
                if (result.opacity.constant < 1.f)
                    result.opacity.kind = CodeGenResult::OpacityResult::Kind::constant;
            }
        }
        ...
        user_data->analysis.graph_has_emission = graph_has_emission(*graph) && materialx_element_may_emit(element);
    ...
    size_t calculate_data_size_upper_bound(const mx::VariableBlock& inputs) const
    {
        auto align_up = [](size_t value, size_t alignment)
        {
            FALCOR_ASSERT(alignment > 0);
            return (value + alignment - 1) / alignment * alignment;
        };

        // This size controls whether MaterialData is stored in the fixed-size inline payload or in
        // a separate buffer. It must be an upper bound for every supported target ABI; an
        // underestimate would let Slang place a larger struct in the inline payload. Use the most
        // conservative scalar/vector/matrix alignment used by D3D12, Vulkan, or CUDA and include
        // the maximum possible inter-member and trailing padding. The bound is intentionally
        // independent of declaration order so codegen may reorder fields without invalidating it.
        // Reflection verifies this bound after the target module has been loaded.
        size_t result = 0;
        size_t struct_alignment = 4;
        for (size_t i = 0; i < inputs.size(); ++i) {
            ...
            case mx::TypeDesc::BaseType::BASETYPE_STRING:
                ...
                break;
            default:
                FALCOR_THROW("MTLX unsupported material data base type '{}'.", type_desc.getName());
            }

            const size_t element_count = type_desc.getSize();
            FALCOR_CHECK(element_count > 0, "MTLX unsupported element count for type '{}'.", type_desc.getName());

            size_t alignment = base_type_size;
            if (type_desc.getSemantic() == mx::TypeDesc::SEMANTIC_MATRIX || element_count >= 3)
                alignment = 16;
            else if (element_count == 2)
                alignment = 8;

            // A float3 may occupy only 12 bytes on some targets, but reserving its full 16-byte
            // alignment slot keeps this calculation a portable upper bound. Matrix33 similarly
            // needs three aligned rows/columns rather than nine tightly packed floats.
            const size_t storage_size = align_up(element_count * base_type_size, alignment);
            // Every supported field starts on at least a four-byte boundary. Charge the maximum
            // padding this field could need from such a boundary, regardless of field order.
            result += storage_size + alignment - 4;
            struct_alignment = std::max(struct_alignment, alignment);
        }
        // The final field may leave the structure at the least favorable four-byte boundary.
        return result + struct_alignment - 4;
    }
    ...
    ) const
    {
        ...
        std::vector<const mx::ShaderPort*> ordered_inputs;
        ordered_inputs.reserve(inputs.size());
        for (size_t i = 0; i < inputs.size(); ++i)
            ordered_inputs.push_back(inputs[i]);
        if (packed_order) {
            std::sort(
                ordered_inputs.begin(),
                ordered_inputs.end(),
                [](const mx::ShaderPort* lhs, const mx::ShaderPort* rhs)
                {
                    const size_t lhs_alignment = material_data_alignment_rank(lhs->getType());
                    const size_t rhs_alignment = material_data_alignment_rank(rhs->getType());
        ...
        for (const mx::ShaderPort* input : ordered_inputs) {
            ...
            // Public uniforms are runtime material data. Their authored values are copied into the
            // instance buffer on the CPU and must not specialize generated Slang.
            else if (input->getType() != mx::Type::FILENAME && inputs.getName() != k_public_uniforms)
    ...
    ) const
    {
        ...
        std::vector<const mx::ShaderPort*> public_uniforms;
        const mx::VariableBlock& public_uniform_block = stage.getUniformBlock(k_public_uniforms);
        public_uniforms.reserve(public_uniform_block.size());
        for (size_t i = 0; i < public_uniform_block.size(); ++i)
            public_uniforms.push_back(public_uniform_block[i]);
        std::sort(
            public_uniforms.begin(),
            public_uniforms.end(),
            [](const mx::ShaderPort* lhs, const mx::ShaderPort* rhs)
            {
                return lhs->getVariable() < rhs->getVariable();
        ...
        for (const mx::ShaderPort* input : public_uniforms) {
        ...
        auto graph_struct_user_data = context.getUserData<CodegenUserData>("mtlx");
        FALCOR_CHECK(graph_struct_user_data, "MTLX emit_graph_struct missing user data.");
    ...
    void emit_material_structs(
        ...
        const CodeGenResult::OpacityResult opacity,
        ...
        emitter::emit_material_structs(
            *this,
            ...
            emitter::MaterialStructsDesc{
                ...
                graph_has_emission,
                opacity,
                calculate_data_size_upper_bound(stage.getUniformBlock(k_public_uniforms)),
            },
            stage
        );
    }
    ...
    MxParamInfo make_param_info(mx::ShaderPort* input_socket, const std::shared_ptr<CodegenUserData>& user_data) const
    {
        ...
        const auto pre_canonical_name = user_data->analysis.pre_canonical_input_names.find(input_socket);
        info.param_name = pre_canonical_name != user_data->analysis.pre_canonical_input_names.end()
            ? pre_canonical_name->second
            : input_socket->getName();
        info.binding_name = input_socket->getVariable();
        ...
        if (input_socket->getType() == mx::Type::FILENAME) {
            ...
            if (pre_canonical_name == user_data->analysis.pre_canonical_input_names.end())
                info.param_name = input_socket->getVariable();
            mx::ShaderNode* node = input_socket->getNode();
            // TODO(mtlx): the four get_input_value calls below are applied
            ...
            info.is_editable = user_data->inputs.desc->is_editable_param(input_socket->getName());
        }
...
struct PreparedCodegenContext {
        ...
        , doc(std::move(doc_))
    {
        ...
        shader_name = desc.class_compilation ? "material_class" : base_name;
        ...
        shader_generator
            = GenslangPtShaderGenerator::create(nullptr, desc.use_slang_derivatives, desc.class_compilation);
        context = std::make_unique<mx::GenContext>(shader_generator);
        register_source_paths(*context);
        ...
        // Falcor shades all materials in linear Rec.709. Always generate MaterialX input transforms into that
        // common renderer working space, independent of the document's authored working space.
        context->getOptions().targetColorSpaceOverride = k_falcor_working_color_space;
        context->getOptions().targetDistanceUnit = "meter";
        context->pushUserData("mtlx", user_data);

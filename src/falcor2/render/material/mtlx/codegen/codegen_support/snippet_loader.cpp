...
void scan_for_leftover_tokens(const std::string& source, std::string_view snippet_name)
{
    ...
    if (std::regex_search(source, match, leftover_pattern)) {
        FALCOR_THROW(
            "MTLX render_snippet('{}'): leftover token '{}' in rendered source (no value supplied).",
            snippet_name,
            match.str()
        );
    }
...
{
#if !defined(MTLX_SNIPPET_PATH)
    FALCOR_THROW("MTLX rewrite snippet path is not configured.");
    return {};
#else
    ...
    const std::filesystem::path path = std::filesystem::path(MTLX_SNIPPET_PATH) / key;
    std::ifstream stream(path, std::ios::binary);
    FALCOR_CHECK(stream.good(), "Unable to read MTLX rewrite Slang snippet '{}'.", path.string());

    std::stringstream buffer;
    buffer << stream.rdbuf();
    FALCOR_CHECK(!stream.bad(), "Failed while reading MTLX rewrite Slang snippet '{}'.", path.string());
...
std::string render_snippet(std::string_view snippet_name, const TokenMap& tokens)
{
    ...
    for (const auto& [key, value] : tokens) {
        FALCOR_CHECK(
            ...
            "MTLX render_snippet('{}'): invalid token key '{}'. Keys must match ^\\$Mx[A-Za-z0-9]+$.",
            snippet_name,
            key
        );

...
{
    constexpr std::string_view k_module_marker = "implementing mtlx_snippets;";
...
void replace_zeltner_sheen_factory_calls(std::string& source, const CodeGenDesc& desc)
{
    if (desc.zeltner_sheen_coefficients == ZeltnerSheenCoefficientMode::analytic_fit)
        return;
    FALCOR_CHECK(
        desc.zeltner_sheen_coefficients == ZeltnerSheenCoefficientMode::lut,
    ...
    replace_all(source, "create_mx_sheen(", "create_mx_sheen_lut(");
}
...
void mark_generated_void_methods_mutating(std::string& source)
{
    ...
    // Advanced Mtlx layering implements the 1.38 generic material instance weight

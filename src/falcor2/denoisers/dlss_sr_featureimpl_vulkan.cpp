...
VkFormat to_vulkan_format(sgl::Format format)
{
    ...
    default:
        FALCOR_THROW("Unsupported Vulkan DLSS-SR texture format: {}", format);
    }
}
...
VulkanTextureResource make_vulkan_resource(ref<sgl::Texture> texture, bool read_write, const char* name)
{
    ...
    FALCOR_CHECK(
        ...
        "Vulkan DLSS-SR evaluation {} must expose a VkImage.",
        name
    );
    ...
    FALCOR_CHECK(
        ...
        "Vulkan DLSS-SR evaluation {} must expose a VkImageView.",
        name
    );
...
class VulkanDLSSSRFeatureImpl final : public DLSSSRFeature::Impl {
public:
    /// Create the Vulkan NGX DLSS-SR feature on an active Vulkan command buffer.
    VulkanDLSSSRFeatureImpl(ref<NGX> ngx, DLSSSRFeatureDesc desc)
    ...
    ~VulkanDLSSSRFeatureImpl() override { release_resources(); }

    /// Record a Vulkan NGX DLSS-SR evaluation callback.
    ...
    ) override
    {
        ...
        command_encoder->execute_callback(
            [this, resources = std::move(resources), owner = std::move(owner)](sgl::NativeHandle native_handle)
            {
                ...
                VulkanTextureResource exposure;
                if (resources.exposure)
                    exposure = make_vulkan_resource(resources.exposure, false, "exposure");

                NVSDK_NGX_VK_DLSS_Eval_Params eval_params{};
                eval_params.Feature.pInColor = &color.resource;
                eval_params.Feature.pInOutput = &output.resource;
                ...
                if (resources.exposure)
                    eval_params.pInExposureTexture = &exposure.resource;
                ...
                set_last_evaluate_result(NGX_VULKAN_EVALUATE_DLSS_EXT(
...
private:
    ...
    void initialize()
    {
        ...
        FALCOR_CHECK(m_vulkan_device, "Vulkan DLSS-SR feature creation requires a VkDevice.");
        ...
        FALCOR_CHECK(
            ...
            "Vulkan DLSS-SR parameter allocation failed: {}",
        ...
        command_encoder->execute_callback(
            [&](sgl::NativeHandle native_handle)
            {
                ...
                if (native_handle.type() != sgl::NativeHandleType::VkCommandBuffer || !native_handle.value()) {
                    callback_error = "Vulkan DLSS-SR feature creation requires an active VkCommandBuffer.";
                    return;
                }

                NVSDK_NGX_DLSS_Create_Params create_params = make_create_params();
                create_result = NGX_VULKAN_CREATE_DLSS_EXT1(
                    m_vulkan_device,
                    reinterpret_cast<VkCommandBuffer>(native_handle.value()),
                    1,
                    1,
        ...
        FALCOR_CHECK(callback_called, "Vulkan DLSS-SR feature creation callback was not executed.");
        FALCOR_CHECK(callback_error.empty(), "{}", callback_error);
        FALCOR_CHECK(
            create_result == NVSDK_NGX_Result_Success && m_feature_handle,
            "Vulkan DLSS-SR feature creation failed: {}",
...
{
    return std::make_unique<VulkanDLSSSRFeatureImpl>(std::move(ngx), desc);
}

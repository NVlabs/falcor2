...
class CameraController:
    ...
    def __init__(self, camera: f2.Camera):
        super().__init__()
        self.camera = camera
        self.on_disable_cursor: Optional[Callable[[], None]] = None
        self.on_enable_cursor: Optional[Callable[[], None]] = None
        ...
        self._key_state = {key: False for key in CameraController.MOVE_KEYS}
        self._shift_down = False
        self._ctrl_down = False

    def _get_transform(self) -> f2.Transform:
        """Get a copy of the camera entity's transform."""
        return f2.Transform(self.camera.entity.transform)

    def _set_transform(self, transform: f2.Transform):
        """Set the camera entity's transform."""
    ...
    def update(self, dt: float) -> bool:
        changed = False
        if self._state == State.IDLE:
            changed = self._update_scroll()
        elif self._state == State.FIRST_PERSON:
            changed = self._update_first_person(dt)
        elif self._state == State.DRAG:
            changed = self._update_drag()
    ...
    def _update_scroll(self) -> bool:
        ...
        rot = spy.math.matrix_from_quat(transform.rotation)
        fwd = -rot.get_col(2)
    ...
    def _update_first_person(self, dt: float) -> bool:
        ...
        if self._shift_down:
            speed *= CameraController.MOVE_FAST_FACTOR
        if self._ctrl_down:
            speed *= CameraController.MOVE_SLOW_FACTOR
        ...
        if spy.math.length(move_delta) > 0:
            ...
            transform.translation = pos + offset * dt
            self._set_transform(transform)
            changed = True
        ...
        if spy.math.length(self._mouse_delta) > 0:
            ...
            horiz = spy.math.quat_from_angle_axis(
                -self._mouse_delta.x * self.rotate_speed,
                spy.float3(0, 1, 0),
            )
            vert = spy.math.quat_from_angle_axis(
                -self._mouse_delta.y * self.rotate_speed,
                spy.float3(1, 0, 0),
            )
            new_rot = spy.math.mul(
                spy.math.mul(horiz, current_rot),
                vert,
            )
    ...
    def _update_drag(self) -> bool:
        ...
        if spy.math.length(self._mouse_delta) > 0:
            ...
            transform.translation = pos + offset * 0.001
            self._set_transform(transform)
            changed = True
    ...
    def _disable_cursor(self):
        if self.on_disable_cursor is not None:
            self.on_disable_cursor()

    def _enable_cursor(self):
        if self.on_enable_cursor is not None:

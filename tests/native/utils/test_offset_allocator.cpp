...
TEST_SUITE_BEGIN("offset_allocator");

TEST_CASE("small-float")
{
    SUBCASE("uint_to_float")
    {
        ...
        uint32_t precise_number_count = 17;
        for (uint32_t i = 0; i < precise_number_count; i++) {
            uint32_t round_up = SmallFloat::uint_to_float_round_up(i);
            uint32_t round_down = SmallFloat::uint_to_float_round_down(i);
            REQUIRE(i == round_up);
            REQUIRE(i == round_down);
        }
        ...
        for (uint32_t i = 0; i < sizeof(test_data) / sizeof(NumberFloatUpDown); i++) {
            NumberFloatUpDown v = test_data[i];
            uint32_t round_up = SmallFloat::uint_to_float_round_up(v.number);
            uint32_t round_down = SmallFloat::uint_to_float_round_down(v.number);
            REQUIRE(round_up == v.up);
            REQUIRE(round_down == v.down);
        }
    }

    SUBCASE("float_to_uint")
    {
        // Denorms, exp=1 and exp=2 + mantissa = 0 are all precise.
        // NOTE: Assuming 8 value (3 bit) mantissa.
        // If this test fails, please change this assumption!
        uint32_t precise_number_count = 17;
        for (uint32_t i = 0; i < precise_number_count; i++) {
            uint32_t v = SmallFloat::float_to_uint(i);
            REQUIRE(i == v);
        }
        ...
        for (uint32_t i = 0; i < 240; i++) {
            uint32_t v = SmallFloat::float_to_uint(i);
            uint32_t round_up = SmallFloat::uint_to_float_round_up(v);
            uint32_t round_down = SmallFloat::uint_to_float_round_down(v);
            REQUIRE(i == round_up);
            REQUIRE(i == round_down);
            // if ((i%8) == 0) printf("\n");
            // printf("%u->%u ", i, v);
...
TEST_CASE("alloc")
{
    ...
    SUBCASE("simple")
    {
        ...
        OffsetAllocator::Allocation validate_all = allocator.allocate(1024 * 1024 * 256);
        REQUIRE(validate_all.offset == 0);
    ...
    SUBCASE("reuse complex")
    {
        ...
        OffsetAllocator::StorageReport report = allocator.storage_report();
        REQUIRE(report.total_free_space == 1024 * 1024 * 256 - 3456 - 2345 - 456 - 512);
        REQUIRE(report.largest_free_region != report.total_free_space);
    ...
    SUBCASE("zero fragmentation")
    {
        ...
        OffsetAllocator::StorageReport report = allocator.storage_report();
        REQUIRE(report.total_free_space == 0);
        REQUIRE(report.largest_free_region == 0);
        ...
        OffsetAllocator::StorageReport report2 = allocator.storage_report();
        REQUIRE(report2.total_free_space == 1024 * 1024 * 256);
        REQUIRE(report2.largest_free_region == 1024 * 1024 * 256);
